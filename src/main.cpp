...
void setup()
{
  ...
  server.on("/tv", HTTP_GET, [](AsyncWebServerRequest *request)
            { request->send_P(200, "text/html", tv_html, processor); });
  ...
  server.on("/v1/scale/data", HTTP_GET, [](AsyncWebServerRequest *request)
            {
              request->send(200, "application/json", scale.getJSON().c_str());
              Serial.println(scale.getJSON().c_str()); });
